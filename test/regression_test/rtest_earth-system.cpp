...
void run(std::string const &sim_name, std::fstream &benck_mark_file) {
    auto system = earth_system<simulation>();

    double t_end = 1000_month;

    auto rms_err = basic_error_test<simulation>("earth-" + sim_name, t_end, 1e-15, system);
    ...
    std::fstream err_stream{"earth-" + sim_name + ".scale", std::ios::out};
...
int main(int argc, char **argv) {
    ...
    std::fstream benck_mark_file{"earth-benchmark.txt", std::ios::out};
