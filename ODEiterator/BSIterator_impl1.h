...
namespace SpaceH
{
    ...
    private:
        ...
        Scalar prepareForNewIteration(Scalar H, size_t k, bool lastRejection);
    };
        ...
        for(;;)
        {
            for(size_t k = 0 ; k <= iterDepth + 1 ; ++k)
            {
                ...
                if(k == iterDepth - 1 || k == iterDepth || k == iterDepth + 1)
                {
                    ...
                    {
                        reject = false;
                        H = prepareForNewIteration(H, k, reject);
    ...
    {
        if(error != 0)
            return  s1 * pow(s2 / error, 1.0 / (2 * k + 1));
        else
            return 1.0 / fmin[k];
    }
     ...
     */
    template <typename ParticSys, typename Integrator>
    typename ParticSys::type::Scalar BSIterator<ParticSys, Integrator>::prepareForNewIteration(Scalar H, size_t k, bool lastRejection)
    {
        Scalar newH = 0;
        
        if(k == iterDepth - 1)
        {
            iterDepth = SpaceH::min(MaxDepth - 1, SpaceH::max(3, k) );
            newH      = SpaceH::max(H*fmin[iterDepth] / s4, SpaceH::min( 1.0*macroStepLength[iterDepth], H / fmin[iterDepth]));
            
            if(work[k - 1] < 0.8 * work[k] && k >= 2)
            {
                iterDepth = SpaceH::min(MaxDepth - 1, SpaceH::max(3, k - 1) );
                newH      = SpaceH::max(H*fmin[iterDepth] / s4, SpaceH::min( 1.0*macroStepLength[iterDepth], H  / fmin[iterDepth]));
            }
            else if(work[k] < 0.9 * work[k - 1] || iterDepth <= 2)
            {
                iterDepth = SpaceH::min(MaxDepth - 1, SpaceH::max(3, k + 1) );
                newH      = SpaceH::max(H*fmin[iterDepth] / s4, SpaceH::min( macroStepLength[iterDepth - 1] * cost[iterDepth] / static_cast<Scalar>(cost[iterDepth - 1]), H / fmin[iterDepth]));
            }
        }
        else if(k == iterDepth)
        {
        ...
        else if(k == iterDepth + 1)
        {
            ...
            {
                iterDepth = SpaceH::min(MaxDepth - 1, SpaceH::max(3, k - 2) );
                newH      = SpaceH::max(H*fmin[iterDepth] / s4, SpaceH::min( 1.0*macroStepLength[iterDepth], H / fmin[iterDepth]));
            }
            else if(!lastRejection && work[k] < 0.9 * work[k - 1] )
            {
