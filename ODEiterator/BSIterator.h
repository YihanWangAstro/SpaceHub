...
typename ParticSys::type::Scalar BSIterator<ParticSys, Integrator>::calcuError(size_t k) const
{
    ...
    Scalar max_err = 0;
     ...
     for(size_t i = 0 ; i < size; ++i)
     {
         ...
         Scalar scale = SpaceH::max(SpaceH::abs(extrapTab[left][i] + initState_[i]), SpaceH::abs(extrapTab[center][i] + initState_[i]));
...
{
    if(error != 0)
        return SpaceH::max(stepsize_limiter[k] / 4, SpaceH::min( 0.9*pow(0.95 / error, err_expon_[k]), 1.0/stepsize_limiter[k] ) );
    else
