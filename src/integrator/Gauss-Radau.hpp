...
namespace space::integrator {
    ...
    template <typename Tab1, typename Tab2>
    void Radau::transfer_G_to_B(const Tab1 &G, Tab2 &B) {
    ...
    template <typename Tab1, typename Tab2>
    void Radau::transfer_B_to_G(const Tab1 &B, Tab2 &G) {
        for (size_t stage = 0; stage < 7; ++stage) {
    ...
    void GaussRadau<TypeSystem>::integrate_to(ParticleSys &particles, Scalar step_size, size_t stage) {
        ...
        auto h_n = Radau::step_sequence(stage);
    ...
    template <typename TypeSystem>
    void GaussRadau<TypeSystem>::update_B_table(const ScalarArray &acc0, const ScalarArray &acc, size_t stage) {
        calc::array_sub(tmp_array_, acc, acc0);
        calc::array_scale(tmp_array_, tmp_array_, Radau::G_tab(stage, 0));
        for (size_t j = 0; j < stage; ++j) {
            calc::array_advance(tmp_array_, g_tab_[j], -Radau::G_tab(stage, j + 1));
        }
        ...
        for (size_t j = 0; j < stage; ++j) {
            ...
            calc::array_div_scale(tmp_array_, tmp_array_, Radau::RR_tab(stage, j + 1));
        }
        ...
        for (size_t i = 0; i <= stage; ++i) {
            calc::array_advance(b_tab_[i], dg_array_, Radau::G2B_tab(stage, i));
        }
    }
    ...
    void GaussRadau<TypeSystem>::predict_new_B(Scalar step_ratio) {
        ...
        for (size_t i = 0; i < final_point; ++i) {
            ...
            calc::array_scale(old_b_tab_[i], b_tab_[6], Q[i] * Radau::B_tab(6, i));
            for (size_t j = 6; j > i; --j) {
                calc::array_advance(old_b_tab_[i], b_tab_[j - 1], Q[i] * Radau::B_tab(j - 1, i));
            }
            //  calc::array_scale(old_b_tab_[i], old_b_tab_[i], Q[i]);
            calc::array_add(b_tab_[i], old_b_tab_[i], tmp_array_);
        }

        Radau::transfer_B_to_G(b_tab_, g_tab_);
    }
}  // namespace space::integrator
