...
#ifndef SPACEHUB_THREAD_POOL_H
#define SPACEHUB_THREAD_POOL_H
...
namespace space::multiThread {
  ...
  public:
    ...
    ThreadPool(size_t max_thread, size_t max_tasks) : stop_(false), max_tasks_(max_tasks) {
    ...
    auto commit(Func &&func, Args &&...args) {
      ...
      std::future<ReturnType> result = job->get_future();
      {
        ...
        if (stop_ || task_num > max_tasks_)
          throw std::runtime_error("Task queue is full/stoped!\n");
