    ...
    auto impl_iterate(U &particles, typename U::Scalar macro_step_size) -> typename U::Scalar {
      ...
      for (size_t k = 0; k < max_iter_; ++k) {
        ...
        if (in_converged_window()) {
          ...
          Scalar new_iter_h = step_controller_.next_step_size((integrator::GaussDadau::order - 1) / 2, iter_h, error);

          if (error < 1) {
            integrator_.integrate_to(particles, iter_h, integrator::GaussDadau::final_point);
