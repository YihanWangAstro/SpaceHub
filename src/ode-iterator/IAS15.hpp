...
namespace space::ode_iterator {
    ...
    class IAS15 {
       ...
       private:
        ...
        Scalar calc_step_error(concepts::ScalarContainer auto const& dy_h, concepts::ScalarContainer auto const& b6,
                               concepts::ParticleSystem auto const& ptc, Scalar step_size) const;
...
#define CLASS_IAS15(...)                                                           \
    template <typename Integrator, typename ErrEstimator, typename StepController> \
    __VA_ARGS__ IAS15<Integrator, ErrEstimator, StepController>
    ...
    CLASS_IAS15(auto)::iterate(concepts::ParticleSystem auto& particles, Scalar macro_step_size) -> Scalar {
    ...
    CLASS_IAS15(void)::reset_PC_iteration() { last_PC_error_ = math::max_value<Scalar>::value; }

    CLASS_IAS15(bool)::in_converged_window() {
    ...
    CLASS_IAS15(auto)::calc_step_error(concepts::ScalarContainer auto const& dy_h,
                                       concepts::ScalarContainer auto const& b6,
                                       concepts::ParticleSystem auto const& ptc, Scalar step_size) const -> Scalar {
        using ParticleSystem = remove_cv_all_t<decltype(ptc)>;
        ...
        for (size_t i = 0; i < ptc_num; i++) {
            ...
            if constexpr (HAS_METHOD(ParticleSystem, chain_pos) && HAS_METHOD(ParticleSystem, chain_vel)) {
            ...
            if (norm2(ptc.pos(i)) * 1e-12 > norm2(ptc.vel(i)) * dt2) {
                ...
                if constexpr (ParticleSystem::ext_vel_dep) {
