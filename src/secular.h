...
namespace secular {
    ...
    public:
        ...
        & dxdt) {
            ...
            Vec3d de2dt = coef * (L_in / L_out) / c_out * ((c_in * c_in * dn1n2) * ce2n1 + (5 * de1n2) * ce1e2 -
                                                       (0.5 - 3 * de1e1 + 12.5 * de1n2 * de1n2 -
                                                        2.5 * c_in * c_in * dn1n2 * dn1n2) * cn2e2);
    ...
    private:
        ...
        double calc_a_in(double L_in, double c_in) {
            return a_in_coef * L_in / c_in;
        }
        
        double calc_a_out(double L_out, double c_out) {
            return a_out_coef * L_out / c_out;
        }
    private:
        double m1;
        double m2;
