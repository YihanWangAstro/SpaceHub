...
namespace space::lazy {
    ...
    struct Larray : public Expr<Larray<T, Len, IsSmall>> {
       public:
        ...
        Larray(Larray &&src) noexcept : data_(std::move(src.data_)) { src.data_ = nullptr; }

        template <size_t S>
        Larray(const T (&src)[S]) : data_(new T[Len]) {
            static_assert(S <= Len, "Capacity of initializer exceeds array size!");
        ...
        inline static constexpr size_t size() { return Len; }

        inline const T &operator[](size_t i) const { return data_[i]; }

        inline T &operator[](size_t i) { return const_cast<T &>(static_cast<Larray const &>(*this)[i]); }
        ...
        Larray &operator=(const T &scalar) { std::fill(data_.get(), data_.get() + Len, scalar); }

        template <typename U>
        Larray(const Expr<U> &expr) : data_(new T[Len]) {
            const U &src = expr.cast();
            multi_threads_loop(Len, multi_thread::auto_thread, [&](size_t begin, size_t end) {
                for (size_t i = begin; i < end; ++i) data_[i] = src.eval(i);
            });
        }
        ...
        Larray &operator=(const Expr<U> &rhs_expr) {
            ...
            for(auto & r : results)
                r.get();*/
            multi_threads_loop(Len, multi_thread::auto_thread, [&](size_t begin, size_t end) {
                for (size_t i = begin; i < end; ++i) data_[i] = rhs.eval(i);
            });
            return *this;
        }
        ...
        void sort() { std::sort(data_.get(), data_.get() + Len); }
