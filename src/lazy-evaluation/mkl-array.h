#ifndef SPACEHUB_MKL_ARRAY_H
#define SPACEHUB_MKL_ARRAY_H
...
struct MKLArray {
  static_assert(std::is_same<T, double>::value || std::is_same<T, float>::value, "Only float and double are allowed.");

  MKLArray() : data_(new T[Size]) {}

  ~MKLArray() { delete[] data_; }

  MKLArray(const MKLArray &src) : data_(new T[Size]) { memcpy(data_, src.data_, sizeof(T) * Size); }
  ...
  T &operator[](size_t i) { return const_cast<T &>(static_cast<MKLArray const &>(*this)[i]); }
