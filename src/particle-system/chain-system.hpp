...
namespace space::particle_system {
  ...
  template<typename Particles, typename Interactions>
  void ChainSystem<Particles, Interactions>::impl_advance_pos(Scalar step_size, const Coord &velocity) {
    Chain::calc_chain(velocity, chain_vel(), index());
    chain_advance(ptcl_.pos(), chain_pos(), chain_vel(), step_size);
  }

  template<typename Particles, typename Interactions>
  void ChainSystem<Particles, Interactions>::impl_advance_vel(Scalar step_size, const Coord &acceleration) {
