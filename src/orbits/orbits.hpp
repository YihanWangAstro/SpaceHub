...
struct OrbitArgs {
 ...
 public:
  ...
  OrbitArgs(Scalar _m1_, Scalar _m2_, Scalar _p_, Scalar _e_, Variant tilt, Variant LoAN, Variant AoP,
...
struct HyperOrbitArgs : public OrbitArgs<Real> {
 ...
 public:
  ...
  HyperOrbitArgs(Scalar _m1_, Scalar _m2_, Scalar v_inf, Scalar b, Scalar r, Variant tilt, Variant LoAN, Variant AoP) {
    Scalar u = space::consts::G * (_m1_ + _m2_);
    Scalar a = -u / (v_inf * v_inf);
    Scalar e = sqrt(1 + b * b / (a * a));
    Scalar p = a * (1 - e * e);
    Scalar true_anomaly = acos((p - r) / (e * r));
    OrbitArgs<Real>(_m1_, _m2_, p, e, tilt, LoAN, AoP, true_anomaly);
  }
};
