 ...
 */
    ...
    bool BSIterator<ParticSys, Integrator>::checkRejection(Scalar error, size_t k) const {
        ...
        if (k == iterDepth - 1)
            stepLengthRatio = nSteps[k + 1] * nSteps[k + 2] / (Scalar) (nSteps[1] * nSteps[1]);
        else if (k == iterDepth)
            stepLengthRatio = nSteps[k + 1] / (Scalar) nSteps[1];
        else
 ...
 */
    ...
    typename ParticSys::type::Scalar BSIterator<ParticSys, Integrator>::getError(size_t k) const {
        if (k != 0) {
            ...
            for (size_t i = 0; i < size; ++i) {
                Scalar scale = (SpaceH::max(SpaceH::abs(extrapTab[topk][i]), SpaceH::abs(extrapTab[subk][i])) *
                                this->relativeError + this->absoluteError);
 ...
 */
    template<typename ParticSys, typename Integrator>
    typename ParticSys::type::Scalar
    BSIterator<ParticSys, Integrator>::prepareForNewIteration(size_t k, bool lastRejection) {
        Scalar newH = 0;
