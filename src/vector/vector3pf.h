...
struct vec3 {
 public:
  ...
  inline vec3 operator+(const T c) const { return vec3(x + c, y + c, z + c); }
  ...
  inline vec3 abs() const { return vec3(x > -x ? x : -x, y > -y ? y : -y, z > -z ? z : -z); }
  ...
  inline T reNorm() const { return 1.0 / sqrt(x * x + y * y + z * z); }
  inline void setZero() { x = y = z = 0; }
  friend vec3 operator+(const T c, const vec3& v) { return vec3(v.x + c, v.y + c, v.z + c); }
  friend vec3 operator-(const T c, const vec3& v) { return vec3(c - v.x, c - v.y, c - v.z); }
  friend vec3 operator*(const T c, const vec3& v) { return vec3(v.x * c, v.y * c, v.z * c); }
