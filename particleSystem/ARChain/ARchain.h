...
public:
    ...
    /*void printAll(char const *str)
    {
        std::cout << "========================" << str << "===================================\n";
        std::cout << "cartesian:\n";
        std::cout << "pos:\n";
        print(this->dynState.pos);
        std::cout << "vel:\n";
        print(this->dynState.vel);
        std::cout << "time:"  << this->dynState.time << '\n';
        std::cout << "omega:" << this->dynState.omega << '\n';
        std::cout << "bindE:" << this->dynState.bindE << '\n';
        ...
        std::cout << "vel:\n";
        print(chain.vel);
        std::cout << "time:"  << chain.time << '\n';
        std::cout << "omega:" << chain.omega << '\n';
        std::cout << "bindE:" << chain.bindE << '\n';
        
        std::cout << "\nroundoff\n";
        std::cout << "pos:\n";
        print(roundoffErr.pos);
        std::cout << "vel:\n";
        print(roundoffErr.vel);
        std::cout << "time:"  << roundoffErr.time << '\n';
        std::cout << "omega:" << roundoffErr.omega << '\n';
        std::cout << "bindE:" << roundoffErr.bindE << '\n';
...
void ARchain<interact::Newtonian<typename EvolvedData::Scalar>, EvolvedData, Regularitor>::updateChain()
{
    ...
    if(chain::IsDiff(chainIndex, newIndex))
    {
        //printAll("before exchange");
        //Update chain index and update new position chain with old chain
        chain::updateChain(this->chain.pos, chainIndex, newIndex);
        chainIndex = newIndex;
        chain::synChain(this->dynState.vel, chain.vel, chainIndex);
        //printAll("after exchange");
       // exit(0);
    }
}
...
void ARchain<interact::Newtonian<typename EvolvedData::Scalar>, EvolvedData, Regularitor>::updateVelIndepAcc()
{
    ...
    Scalar inv_r  = 1;
    Scalar inv_r3 = 1;
    //printAll("before update acc");
    std::for_each(this->velIndepAcc.begin(), this->velIndepAcc.end(), [](vec3<Scalar>& v)
                  {
                      v.setZero();
                  });
